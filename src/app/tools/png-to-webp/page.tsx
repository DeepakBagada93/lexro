...
export default function PngToWebpConverterPage() {
  ...
  const [quality, setQuality] = useState(0.90); // WEBP quality 0 to 1
  ...
  const handleConvert = () => {
    ...
    img.onload = () => {
      ...
      if (ctx) {
        ...
        try {
          // For WEBP, quality is between 0 and 1.
          // Note: canvas.toDataURL('image/webp') might not be supported in all older browsers (e.g., Safari < 14)
          const webpDataUrl = canvas.toDataURL('image/webp', quality);
          setConvertedImageDataUrl(webpDataUrl);
          toast({ title: "Conversion Successful", description: "Image successfully converted to WEBP." });
        } catch (e) {
            console.error("Canvas toDataURL error for WEBP:", e);
            toast({ title: "Conversion Error", description: "Could not convert image to WEBP. Your browser might not support it or the image is too large.", variant: "destructive" });
            setConvertedImageDataUrl(null);
        }
      } else {
    ...
    img.crossOrigin = "anonymous"; // Important for images from other origins if that were the case
  ...
  const handleDownload = () => {
    if (convertedImageDataUrl && uploadedFile) {
      ...
      toast({ title: "No converted image", description: "Please convert an image to WEBP first.", variant: "destructive" });
    }
  };
      ...
      <main className="flex-grow container mx-auto px-4 py-10 sm:py-16">
        <Card className="max-w-2xl mx-auto shadow-xl">
          <CardHeader>
            ...
            <CardDescription>Convert your PNG images to modern WEBP format for better compression. Adjust quality as needed.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="qualitySlider" className="font-medium">WEBP Quality: {Math.round(quality * 100)}%</Label>
              <Slider
                id="qualitySlider"
                min={0.1}
                max={1}
              ...
              />
              <p className="text-xs text-muted-foreground">WEBP offers good compression. Higher quality means larger file size.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
              ...
              {convertedImageDataUrl && (
                <div className="space-y-2">
                  <h3 className="text-lg font-medium text-center">Converted WEBP</h3>
                  <ImagePreview src={convertedImageDataUrl} alt="Converted WEBP preview" />
                </div>
              )}
            </div>
             {isProcessing && (
